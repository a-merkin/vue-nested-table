...
const emit = defineEmits<{
  ...
  (e: 'cell-move', move: { direction: 'up' | 'down' | 'left' | 'right'; position: CellPosition }): void;
...
const handleKeydown = (e: KeyboardEvent) => {
  if (e.key === 'Enter' || e.key === 'Tab') {
    e.preventDefault();
    emit('cell-move', {
      direction: e.key === 'Enter' ? 'down' : 'right',
      position: { row: props.rowIndex, col: props.colIndex }
    });
  }
};
...
const handlePaste = (e: ClipboardEvent) => {
  ...
  const validDates = values.map(value => {
