...
const cellValues = ref<string[][]>(
  props.initialValues || Array(props.rows).fill(null).map(() => Array(props.cols).fill(''))
);

const { selectedRange, activeCell, isCellSelected, isCellActive, selectCell, moveSelection } = useTableSelection(props.rows, props.cols);
        ...
        const selectedValues = cellValues.value
          .map((row, rowIndex) => 
            row.map((value, colIndex) => 
              isCellSelected(rowIndex, colIndex) ? value : null
            )
          )
          .flat()
          .filter(Boolean);
