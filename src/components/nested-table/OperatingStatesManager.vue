<template>
  <div class="operating-states-manager">
    <div v-for="(state, index) in modelValue" :key="index" class="operating-state-entry">
      <select v-model="state.state_id" class="state-select" @change="validateAndEmit">
      ...
      <input
        ...
        @change="validateAndEmit"
      />
      <button class="remove-button" :disabled="modelValue.length === 1" @click="removeState(index)">
        ✕
      </button>
    </div>
    <button class="add-button" :disabled="!canAddState" @click="addState">
      Добавить состояние
    </button>
